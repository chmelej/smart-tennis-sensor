            ...
            for (int eventType = xmlPullParserNewPullParser.getEventType(); eventType != 1; eventType = xmlPullParserNewPullParser.next()) {
                ...
                if (eventType == 2) {
                    if (NODE_DB_NAME.equals(name)) {
                        litePalAttr.setDbName(xmlPullParserNewPullParser.getAttributeValue("", ATTR_VALUE));
                    } else if (NODE_VERSION.equals(name)) {
                        litePalAttr.setVersion(Integer.parseInt(xmlPullParserNewPullParser.getAttributeValue("", ATTR_VALUE)));
                    } else if (NODE_MAPPING.equals(name)) {
                        litePalAttr.addClassName(xmlPullParserNewPullParser.getAttributeValue("", ATTR_CLASS));
                    } else if (NODE_CASES.equals(name)) {
                        litePalAttr.setCases(xmlPullParserNewPullParser.getAttributeValue("", ATTR_VALUE));
                    }
                }
            }
