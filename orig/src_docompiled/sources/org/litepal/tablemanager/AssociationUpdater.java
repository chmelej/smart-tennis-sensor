...
public abstract class AssociationUpdater extends Creator {
    ...
    protected List<String> getForeignKeyColumns(TableModel tableModel) {
        ...
        for (ColumnModel columnModel : getTableModelFromDB(tableModel.getTableName()).getColumnModels()) {
            ...
            if (isForeignKeyColumnFormat(columnModel.getColumnName()) && !tableModel.containsColumn(columnName)) {
                LogUtil.m11042d(TAG, "getForeignKeyColumnNames >> foreign key column is " + columnName);
    ...
    protected void dropTables(List<String> list, SQLiteDatabase sQLiteDatabase) {
        ...
        for (int i = 0; i < strArr.length; i++) {
            strArr[i] = generateDropTableSQL(list.get(i));
        }
        execute(strArr, sQLiteDatabase);
    }
    ...
    protected void clearCopyInTableSchema(List<String> list) {
        ...
        LogUtil.m11042d(TAG, "clear table schema value sql is " + ((Object) sb));
        execute(new String[]{sb.toString()}, this.mDb);
    }
    ...
    private List<String> findForeignKeyToRemove(TableModel tableModel) {
        ...
        LogUtil.m11042d(TAG, "findForeignKeyToRemove >> " + tableModel.getTableName() + " " + arrayList);
        return arrayList;
    }
    ...
    private List<String> findIntermediateTablesToDrop() {
        ...
        LogUtil.m11042d(TAG, "findIntermediateTablesToDrop >> " + arrayList);
        return arrayList;
    }
    ...
    private String[] getRemoveColumnSQLs(Collection<String> collection, String str) {
        TableModel tableModelFromDB = getTableModelFromDB(str);
        String strGenerateAlterToTempTableSQL = generateAlterToTempTableSQL(str);
        LogUtil.m11042d(TAG, "generateRemoveColumnSQL >> " + strGenerateAlterToTempTableSQL);
        String strGenerateCreateNewTableSQL = generateCreateNewTableSQL(collection, tableModelFromDB);
        LogUtil.m11042d(TAG, "generateRemoveColumnSQL >> " + strGenerateCreateNewTableSQL);
        String strGenerateDataMigrationSQL = generateDataMigrationSQL(tableModelFromDB);
        LogUtil.m11042d(TAG, "generateRemoveColumnSQL >> " + strGenerateDataMigrationSQL);
        String strGenerateDropTempTableSQL = generateDropTempTableSQL(str);
        LogUtil.m11042d(TAG, "generateRemoveColumnSQL >> " + strGenerateDropTempTableSQL);
        return new String[]{strGenerateAlterToTempTableSQL, strGenerateCreateNewTableSQL, strGenerateDataMigrationSQL, strGenerateDropTempTableSQL};
    }
