package p000;

import android.content.Context;
import android.content.Intent;
import android.content.pm.PackageManager;
import android.content.pm.Signature;

/* JADX INFO: loaded from: classes.dex */
class aaa {
    /* JADX INFO: renamed from: a */
    public static boolean m6a(Context context, String str, boolean z) {
        if (z) {
            try {
                return m7a(context, context.getPackageManager().getPackageInfo(str, 64).signatures, z);
            } catch (PackageManager.NameNotFoundException unused) {
                return false;
            }
        }
        C2295zi.m12552d("MicroMsg.SDK.WXMsgImplComm", "ignore wechat app signature validation");
        return true;
    }

    /* JADX INFO: renamed from: a */
    public static boolean m7a(Context context, Signature[] signatureArr, boolean z) {
        String str;
        String str2;
        if (z) {
            for (Signature signature : signatureArr) {
                String lowerCase = signature.toCharsString().toLowerCase();
                C2295zi.m12552d("MicroMsg.SDK.WXMsgImplComm", "check signature:" + lowerCase);
                if (lowerCase.equals("")) {
                    str = "MicroMsg.SDK.WXMsgImplComm";
                    str2 = "pass";
                }
            }
            return false;
        }
        str = "MicroMsg.SDK.WXMsgImplComm";
        str2 = "ignore wechat app signature validation";
        C2295zi.m12552d(str, str2);
        return true;
    }

    /* JADX INFO: renamed from: a */
    public static boolean m8a(Intent intent, String str) {
        String stringExtra;
        return (intent == null || (stringExtra = intent.getStringExtra("wx_token_key")) == null || !stringExtra.equals(str)) ? false : true;
    }
}
